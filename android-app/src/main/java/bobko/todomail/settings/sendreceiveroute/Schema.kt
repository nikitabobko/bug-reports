...
sealed class TextFieldItem<T : Any>(
    ...
    protected open fun onTextChanged(sendReceiveRoute: MutableState<SendReceiveRoute>) {}
    protected open fun isRightSideHintVisible(sendReceiveRoute: SendReceiveRoute) = false
...
class SmtpServerPortTextFieldItem() : TextFieldItem<Int>(
    ...
    override fun isRightSideHintVisible(sendReceiveRoute: SendReceiveRoute) =
        KnownSmtpCredential.findBySmtpServer(sendReceiveRoute)
            ?.let { sendReceiveRoute.credential.smtpServerPort == it.smtpCredential.smtpServerPort } == true
